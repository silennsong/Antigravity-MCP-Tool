...
try:
    ...
    from antigravity_mcp import adapter, config
...
SERVER_INSTRUCTIONS = (
    ...
    "Pass the current project as workspace and follow its applicable AGENTS.md. If "
...
def _model_name(request: DelegationRequest) -> str:
    ...
    if not model:
        raise RequestError(
            f"{variable} is not configured. Run `agy models`, set that variable, "
            "or pass an exact model name if project policy permits it."
        )
    return model
...
def _agy_executable() -> str:
    ...
    if found is None:
        raise RequestError(
            f"Antigravity CLI `{configured}` was not found on PATH. Install/login first."
        )
    return found
...
def _error_result(message: str) -> dict[str, Any]:
    return {
        ...
        "structuredContent": {"rejected": True, "reason": message},
        "isError": True,
    }
